...
namespace SidebarDiagnostics
{
    ...
    public partial class Settings : FlatWindow
    {
        ...
        public Settings(FloatingBar floatingBar)
        {
            _floatingBar = floatingBar;
            
            InitializeComponent();
            
            // Create settings model without sidebar reference
            DataContext = Model = new SettingsModel(null);

            // Position window centered on screen
            WindowStartupLocation = WindowStartupLocation.CenterScreen;
            
            // Show non-modal for floating bar
            Show();
        }

        private async Task Save(bool finalize)
        {
            DockEdge previousDockEdge = Framework.Settings.Instance.DockEdge;
            
            Model.Save();

            await App.Current.Dispatcher.BeginInvoke(DispatcherPriority.ApplicationIdle, (Action)(async () =>
            {
                // Check if we're switching to/from floating mode
                bool switchingToFloating = Framework.Settings.Instance.DockEdge == DockEdge.Floating && previousDockEdge != DockEdge.Floating;
                bool switchingFromFloating = Framework.Settings.Instance.DockEdge != DockEdge.Floating && previousDockEdge == DockEdge.Floating;

                if (switchingToFloating)
                {
                    // Close existing sidebar and open floating bar
                    Sidebar existingSidebar = App.Current.Sidebar;
                    if (existingSidebar != null)
                    {
                        existingSidebar.Close();
                    }
                    new FloatingBar().Show();
                    return;
                }
                else if (switchingFromFloating)
                {
                    // Close floating bar and open sidebar
                    FloatingBar existingFloatingBar = App.Current.FloatingBar;
                    if (existingFloatingBar != null)
                    {
                        existingFloatingBar.Close();
                    }
                    new Sidebar(false, false).Show();
                    return;
                }

                // Normal case - update existing window
                ...
                FloatingBar _floatingBar = App.Current.FloatingBar;
        ...
        {
            try { Hotkey.Enable(); } catch { }
        }

        private void GitHub_Click(object sender, RoutedEventArgs e)
        {
            System.Diagnostics.Process.Start(System.Configuration.ConfigurationManager.AppSettings["RepoURL"]);
        }

        private void Donate_Click(object sender, RoutedEventArgs e)
        {
            System.Diagnostics.Process.Start(System.Configuration.ConfigurationManager.AppSettings["DonateURL"]);
        }

        private void Twitter_Click(object sender, RoutedEventArgs e)
        {
            System.Diagnostics.Process.Start("https://x.com/yousef_dev921");
        }
